...
type AdminTab = 'dashboard' | 'logo' | 'spools' | 'data' | 'feedback' | 'requests';
...
export const AdminPanel: React.FC = () => {
  ...
  // ... (Effect hooks and loaders same as before) ...
  useEffect(() => {
    loadDashboardStats();
    checkLatency();
  ...
  // ... (Logo handlers and Delete User handlers remain same) ...
  ...
  const copyInstallLink = () => { const url = `${window.location.origin}/?install=true`; navigator.clipboard.writeText(url); alert("Installatie link gekopieerd!"); };

  const sqlSetupCode = `-- UPDATE: VOEG KLIPPER KOLOMMEN TOE
alter table public.printers add column if not exists "ipAddress" text;
alter table public.printers add column if not exists "apiKey" text;
alter table public.printers add column if not exists "webcamUrl" text; -- NIEUW: Webcam stream

-- UPDATE: VOEG IMAGE KOLOM TOE AAN OTHER_MATERIALS
alter table public.other_materials add column if not exists image text;

-- NIEUW: OVERIGE MATERIALEN TABEL (Als je deze nog niet hebt)
...
drop policy if exists "Gebruikers beheren eigen materialen" on public.other_materials;
create policy "Gebruikers beheren eigen materialen" on public.other_materials for all using (auth.uid() = user_id);

-- EERDER: MERKEN & MATERIALEN
create table if not exists public.brands (id bigint generated by default as identity primary key, name text unique not null);
create table if not exists public.materials (id bigint generated by default as identity primary key, name text unique not null);
alter table public.brands enable row level security;
alter table public.materials enable row level security;

drop policy if exists "Iedereen mag lezen" on public.brands;
drop policy if exists "Admins schrijven" on public.brands;
create policy "Iedereen mag lezen" on public.brands for select using (true);
create policy "Admins schrijven" on public.brands for insert with check (auth.email() = 'timwillemse@hotmail.com'); 

drop policy if exists "Iedereen mag lezen" on public.materials;
drop policy if exists "Admins schrijven" on public.materials;
create policy "Iedereen mag lezen" on public.materials for select using (true);
create policy "Admins schrijven" on public.materials for insert with check (auth.email() = 'timwillemse@hotmail.com');

create table if not exists public.spool_weights (id bigint generated by default as identity primary key, name text not null, weight numeric not null);
...
drop policy if exists "Admins beheren" on public.spool_weights;
create policy "Iedereen mag lezen" on public.spool_weights for select using (true);
create policy "Admins beheren" on public.spool_weights for all using (auth.email() = 'timwillemse@hotmail.com'); 

create table if not exists public.global_settings (key text primary key, value text);
alter table public.global_settings enable row level security;
drop policy if exists "Iedereen kan settings lezen" on public.global_settings;
drop policy if exists "Alleen admins kunnen schrijven" on public.global_settings;
create policy "Iedereen kan settings lezen" on public.global_settings for select using (true);
create policy "Alleen admins kunnen schrijven" on public.global_settings for all using (auth.email() = 'timwillemse@hotmail.com'); 

alter table public.feedback add column if not exists is_read boolean default false;
drop policy if exists "Admins mogen updaten" on public.feedback;
create policy "Admins mogen updaten" on public.feedback for update using (true);
`;

  const handleCopySql = () => { navigator.clipboard.writeText(sqlSetupCode); setCopiedSql(true); setTimeout(() => setCopiedSql(false), 2000); };

  // ... (Render Logic remains the same, simplified for brevity) ...
  return (
       ...
       <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 space-y-6">
             <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-2 flex overflow-x-auto gap-2 scrollbar-hide">
                ...
                <button onClick={() => setActiveTab('logo')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${activeTab === 'logo' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-900'}`}><ImageIcon size={18}/> Logo</button>
                ...
                <div className="space-y-4">
                   ...
                   <div className="bg-slate-900 text-slate-300 p-4 rounded-xl font-mono text-xs overflow-x-auto border border-slate-700 relative group">
                        <div className="flex justify-between items-center mb-2 sticky left-0 right-0">
                           <p className="text-slate-500 font-bold uppercase flex items-center gap-2"><Database size={14}/> SQL Setup Helper</p>
                        ...
                        <pre>{sqlSetupCode}</pre>
                   </div>
                </div>
             )}
             
             {/* ... (Other tabs kept as is, just wrapped in similar logic) ... */}
