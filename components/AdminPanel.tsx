...
const MASTER_SQL = `-- 1. SPOOL DATABASE SCHEMA UPDATE
...
-- 2. STATUS COLUMNS (is_ordered)
do $$ 
begin 
  if not exists (select 1 from information_schema.columns where table_name='filaments' and column_name='is_ordered') then
    alter table public.filaments add column is_ordered boolean default false;
  end if;
  if not exists (select 1 from information_schema.columns where table_name='other_materials' and column_name='is_ordered') then
    alter table public.other_materials add column is_ordered boolean default false;
  end if;
end $$;

-- 3. OTHER TABLES & POLICIES
...
drop policy if exists "Users manage own requests" on public.deletion_requests;
create policy "Users manage own requests" on public.deletion_requests for all using (auth.uid() = user_id);
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const handleAiCatalogParse = async () => {
     ...
     } catch (e: any) { alert("Analysis failed."); } finally { setIsParsingCatalog(false); }
  };
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-fade-in flex flex-col gap-8">
       <div className="space-y-6 w-full">
          ...
          <div className="min-h-[400px]">
                ...
                <div className="space-y-6">
                   <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                         <h3 className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Users</h3>
                         <p className="text-3xl font-black text-slate-800 dark:text-white">{tableCounts.totalUsers}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                         <h3 className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">PRO Members</h3>
                      ...
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                         <h3 className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Materials</h3>
                         <p className="text-3xl font-black text-slate-800 dark:text-white">{tableCounts.materials}</p>
                      </div>
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                         <h3 className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Feedback</h3>
                   ...
                   <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
                      <div className="flex-1">
                         ...
                         <p className="text-slate-500 text-sm">Copy the complete SQL code to correctly create or update all tables and RLS policies in Supabase at once.</p>
                      </div>
                      <button onClick={handleCopySql} className={`px-8 py-4 rounded-2xl font-black transition-all flex items-center gap-2 shadow-lg ${sqlCopied ? 'bg-emerald-500 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}>
                         {sqlCopied ? <Check/> : <Copy/>} {sqlCopied ? 'Copied!' : 'Copy SQL'}
                      </button>
                   </div>
                </div>
             )}
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                   <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex flex-col md:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-3">
                         ...
                         <h3 className="font-bold text-slate-800 dark:text-white">Users ({filteredUsers.length})</h3>
                      </div>
                   ...
                   <div className="overflow-x-auto">
                      <table className="w-full text-left">
                         <thead className="bg-slate-50 dark:bg-slate-900">
                            <tr>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase">User</th>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase text-center">Statistics</th>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase text-center">Status</th>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase text-right">Registered</th>
                            </tr>
                         </thead>
                ...
                <div className="space-y-6">
                   <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-6">
                      <div className="flex justify-between items-center">
                         <h3 className="font-bold text-slate-800 dark:text-white flex items-center gap-2"><Plus size={20} className="text-emerald-500"/> Add Manually</h3>
                      ...
                      <form onSubmit={handleAddSpool} className="grid grid-cols-1 md:grid-cols-5 gap-3">
                         <div className="md:col-span-2"><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Brand</label><input type="text" value={newSpool.brand} onChange={e => setNewSpool({...newSpool, brand: e.target.value})} placeholder="e.g. Bambu Lab" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" /></div>
                         <div><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Size</label><input type="text" value={newSpool.size} onChange={e => setNewSpool({...newSpool, size: e.target.value})} placeholder="e.g. 1000g" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" /></div>
                         <div><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Material</label><input type="text" value={newSpool.spool_material} onChange={e => setNewSpool({...newSpool, spool_material: e.target.value})} placeholder="e.g. Plastic" className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" /></div>
                         <div className="relative"><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Empty weight</label><input type="number" value={newSpool.weight} onChange={e => setNewSpool({...newSpool, weight: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl p-3 outline-none focus:ring-2 focus:ring-blue-500 dark:text-white" /><span className="absolute right-3 top-9 text-slate-400 text-xs">g</span></div>
                         <div className="md:col-span-5 pt-2"><button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-all shadow-md">Add to Database</button></div>
                      </form>
                   </div>

                   {showAiImport && (
                      <div className="bg-indigo-50 dark:bg-indigo-900/10 rounded-2xl border border-indigo-200 p-6 animate-fade-in space-y-4">
                         <div className="flex justify-between items-center"><h3 className="font-bold text-indigo-900 dark:text-indigo-400">AI Catalog Import</h3><button onClick={() => setShowAiImport(false)}><X size={20}/></button></div>
                         {!parsedSpools.length ? (
                            <div className="space-y-4">
                               <textarea value={importText} onChange={e => setImportText(e.target.value)} placeholder="Paste text from Printables/catalog here..." className="w-full h-48 bg-white dark:bg-slate-900 border border-indigo-200 rounded-xl p-4 font-mono text-sm" />
                               <button onClick={handleAiCatalogParse} disabled={isParsingCatalog || !importText.trim()} className="w-full py-4 bg-indigo-600 text-white font-black rounded-xl shadow-lg transition-all flex items-center justify-center gap-2">{isParsingCatalog ? <Loader2 className="animate-spin" /> : 'Analyze with AI'}</button>
                            </div>
                         ) : (
                            <div className="space-y-4">
                               <div className="flex justify-between items-center"><span className="text-sm font-bold">{parsedSpools.length} found</span><div className="flex gap-2"><button onClick={() => setParsedSpools([])} className="px-4 py-2 text-xs bg-slate-100 rounded-lg">Reset</button><button onClick={handleBulkImport} className="px-4 py-2 text-xs bg-green-600 text-white rounded-lg">Bulk Import ({parsedSpools.filter(s=>s.selected).length})</button></div></div>
                      ...
                      <div className="max-h-[600px] overflow-y-auto">
                         <table className="w-full text-left">
                            <thead className="bg-slate-50 dark:bg-slate-900 sticky top-0 z-10">
                               <tr>
                                  <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Brand</th>
                                  <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Size</th>
                                  <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Material</th>
                                  <th className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center">Weight</th>
             ...
             {activeTab === 'feedback' && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                   <div className="p-6 border-b bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center"><h3 className="font-bold dark:text-white flex items-center gap-3"><MessageSquare size={20}/> Feedback List</h3></div>
                   <div className="overflow-x-auto">
                      <table className="w-full text-left">
                         <thead className="bg-slate-50 dark:bg-slate-900">
                            <tr>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase w-12">Status</th>
                               <th className="p-4 text-[10px] font-black text-slate-500 uppercase">Message</th>
             ...
             {activeTab === 'logo' && (
                <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col items-center">
                   ...
                   <button onClick={handleLogoUpload} disabled={!previewUrl || isUploading} className="w-full max-w-xs bg-indigo-600 text-white font-bold py-3 rounded-xl shadow-lg flex items-center justify-center gap-2">{isUploading ? <Loader2 className="animate-spin"/> : <Save/>} Save Logo</button>
                </div>
             )}
             ...
             {activeTab === 'data' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <h3 className="font-bold mb-4 dark:text-white flex items-center gap-2"><Tag size={20}/> Brands</h3>
                      <div className="flex gap-2 mb-4"><input type="text" value={newBrand} onChange={e => setNewBrand(e.target.value)} placeholder="New brand..." className="flex-1 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl dark:text-white border" /><button onClick={async () => { if(newBrand) { await supabase.from('brands').insert({name: newBrand}); setNewBrand(''); loadBrands(); } }} className="bg-blue-600 text-white px-4 rounded-xl">Add</button></div>
                   ...
                   <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <h3 className="font-bold mb-4 dark:text-white flex items-center gap-2"><Layers size={20}/> Materials</h3>
                      <div className="flex gap-2 mb-4"><input type="text" value={newMaterial} onChange={e => setNewMaterial(e.target.value)} placeholder="New material..." className="flex-1 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl dark:text-white border" /><button onClick={async () => { if(newMaterial) { await supabase.from('materials').insert({name: newMaterial}); setNewMaterial(''); loadMaterials(); } }} className="bg-blue-600 text-white px-4 rounded-xl">Add</button></div>
       ...
       <div className="bg-slate-900 rounded-[32px] p-8 text-white flex flex-wrap gap-8 items-center justify-between border border-slate-800">
          ...
          <div className="flex flex-wrap gap-10">
             ...
             <div className="flex flex-col"><span className="text-[10px] text-slate-500 font-black uppercase">AI Engine</span><span className={`font-bold ${isAiConfigured ? 'text-emerald-400' : 'text-red-400'}`}>{isAiConfigured ? 'Gemini 3 Flash' : 'Key Missing'}</span></div>
             <div className="flex flex-col"><span className="text-[10px] text-slate-500 font-black uppercase">App Version</span><span className="font-mono">{t('version') || '2.1.35'}</span></div>
          </div>
       </div>
    </div>
  );
