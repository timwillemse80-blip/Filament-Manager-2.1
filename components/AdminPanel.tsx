...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const handleAddSpool = async (e: React.FormEvent) => {
    ...
    try {
       await supabase.from('spool_weights').insert({ brand: newSpool.brand, size: newSpool.size, spool_material: newSpool.spool_material, weight: Number(newSpool.weight) });
  ...
  const handleUpdateSpool = async (id: number) => {
    ...
    try {
       await supabase.from('spool_weights').update({ brand: editingSpoolData.brand, size: editingSpoolData.size, spool_material: editingSpoolData.spool_material, weight: Number(editingSpoolData.weight) }).eq('id', id);
    ...
    <div className="max-w-7xl mx-auto pb-20 animate-fade-in flex flex-col gap-8">
                ...
                <div className="space-y-6">
                   <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col gap-6">
                      ...
                      <form onSubmit={handleAddSpool} className="grid grid-cols-1 md:grid-cols-5 gap-3">
                         <div className="md:col-span-2"><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Merk</label><input type="text" value={newSpool.brand} onChange={e => setNewSpool({...newSpool, brand: e.target.value})} placeholder="bv. Bambu Lab" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 p-3 rounded-xl dark:text-white" /></div>
                         <div><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Grootte</label><input type="text" value={newSpool.size} onChange={e => setNewSpool({...newSpool, size: e.target.value})} placeholder="bv. 1000g" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 p-3 rounded-xl dark:text-white" /></div>
                         <div><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Materiaal</label><input type="text" value={newSpool.spool_material} onChange={e => setNewSpool({...newSpool, spool_material: e.target.value})} placeholder="bv. Plastic" className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 p-3 rounded-xl dark:text-white" /></div>
                         <div className="relative"><label className="text-[10px] font-black uppercase text-slate-500 mb-1 block">Leeg gewicht</label><input type="number" value={newSpool.weight} onChange={e => setNewSpool({...newSpool, weight: e.target.value})} className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 p-3 rounded-xl dark:text-white" /><span className="absolute right-3 top-9 text-slate-400 text-xs">g</span></div>
       ...
       <div className="bg-slate-900 rounded-[32px] p-8 text-white flex flex-wrap gap-8 items-center justify-between border border-slate-800">
          ...
          <div className="flex flex-wrap gap-10">
             ...
             <div className="flex flex-col"><span className="text-[10px] text-slate-500 font-black uppercase">App Versie</span><span className="font-mono">2.1.34</span></div>
          </div>
       </div>
    </div>
  );
