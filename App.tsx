...
const AppContent = () => {
  ...
  const handleBackAction = () => {
    ...
    if (showModal) { setShowModal(false); setEditingId(null); setShowLabelOnly(false); return; }
    ...
    if (view === 'dashboard') {
      if (Capacitor.isNativePlatform() && navigator.vibrate) navigator.vibrate(50);
      setShowExitConfirm(true);
    }
  };

  // Capacitor Hardware Back Button Listener (Safe for native)
  useEffect(() => {
    if (!Capacitor.isNativePlatform()) return;
    const backButtonListener = CapacitorApp.addListener('backButton', handleBackAction);
  ...
  // Passieve PWA Back Listener (Zonder pushState die crash veroorzaakt op sommige sandboxes)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const checkUpdates = async () => {
      try {
        ...
        if (data.version && data.version !== APP_VERSION) {
          const notes = data.releaseNotes[language] || data.releaseNotes['en'] || "";
          setUpdateInfo({ version: data.version, notes, downloadUrl: data.downloadUrl });
    ...
    CapacitorApp.addListener('appStateChange', ({ isActive }) => {
      if (isActive && session?.user?.id) fetchData(session.user.id);
    });
    ...
    CapacitorApp.addListener('appUrlOpen', (data: any) => {
       ...
       if (url.startsWith('filament://')) {
          ...
          if (shortId) handleSpoolDeepLink(shortId);
       }
    });
  ...
  const handleBatchDelete = async (ids: string[], type: 'filament' | 'material') => {
    ...
    try {
      const table = type === 'filament' ? 'filaments' : 'other_materials';
      await supabase.from(table).delete().in('id', ids); fetchData();
    } catch (e: any) { alert("Batch verwijderen mislukt: " + e.message); }
  };
  ...
  const handleQuickAdjust = async (id: string, amount: number) => {
    ...
    await supabase.from('filaments').update({ weightRemaining: newWeight }).eq('id', id); fetchData();
  };
  ...
  const handleMaterialAdjust = async (id: string, amount: number) => {
    ...
    await supabase.from('other_materials').update({ quantity: newQty }).eq('id', id); fetchData();
  };
  ...
  return (
    <div className="h-screen w-full overflow-hidden bg-slate-50 dark:bg-slate-900 flex">
      <aside className={`hidden lg:flex flex-col w-72 bg-white dark:bg-slate-950 border-r dark:border-slate-800 transition-all duration-300 ${isDesktopSidebarOpen ? 'translate-x-0' : '-translate-x-full w-0 overflow-hidden'}`}>
        ...
        <SidebarContent view={view} setView={setView} filaments={filaments} lowStockCount={totalLowStock} onClose={() => {}} t={t} isAdmin={isAdmin} isPremium={isPremium} onBecomePro={() => setShowProModal(true)} onOpenShowcase={() => setShowShowcaseModal(true)} adminBadgeCount={adminBadgeCount} avgRating={avgRating} />
      </aside>
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        <header className="bg-white dark:bg-slate-950 border-b dark:border-slate-800 h-16 flex items-center px-4 justify-between gap-2">
          ...
          <div className="shrink-0 flex items-center gap-1">
             <button onClick={() => setView('notifications')} className={`p-2 rounded-full relative ${updateInfo ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400' : 'text-slate-400 hover:bg-slate-100'}`}><Bell size={20} className={updateInfo ? 'animate-pulse' : ''} />{updateInfo && <span className="bg-blue-500 border-white border-2 rounded-full w-4 h-4 flex items-center justify-center font-bold text-white text-[10px] absolute top-0 right-0">1</span>}</button>
             <button onClick={() => setIsSnowEnabled(!isSnowEnabled)} className={`p-2 rounded-full ${isSnowEnabled ? 'bg-sky-100 text-sky-600 dark:bg-sky-900/20 dark:text-sky-400' : 'text-slate-400 hover:bg-slate-100'}`}><Snowflake size={20} /></button>
          </div>
        </header>
      ...
      {showModal && <FilamentForm initialData={editingFilament} initialShowLabel={showLabelOnly} locations={locations} suppliers={suppliers} existingBrands={existingBrands} onSave={handleSaveFilament} onSaveLocation={() => fetchData()} onSaveSupplier={() => fetchData()} onCancel={() => { setShowModal(false); setEditingId(null); setShowLabelOnly(false); }} />}
      ...
      {showExitConfirm && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/70 backdrop-blur-md p-6 animate-fade-in">
          <div className="bg-white dark:bg-slate-900 w-full max-sm rounded-[32px] shadow-2xl p-8 text-center border border-slate-200 dark:border-slate-800">
